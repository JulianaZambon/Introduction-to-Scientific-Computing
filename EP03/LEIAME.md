...
Para obter a versão otimizada `ajustePol_`v2`.c` a partir do código original `ajustePol_v1.c`, foram **testadas** as seguintes alterações:
...
Para obter a versão otimizada `ajustePol_`v2`.c` a partir do código original `ajustePol_v1.c`, foram **implementadas** as seguintes alterações:
...
A análise de desempenho foi realizada comparando as duas versões do programa `ajustePol_v1.c` e `ajustePol_`v2`.c` sob condições controladas, utilizando a ferramenta LIKWID.
