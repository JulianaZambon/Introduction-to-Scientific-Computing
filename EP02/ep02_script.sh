...
if [ -t 0 ]; then
    echo "Erro: forneça a entrada via redirecionamento, por exemplo: ./runEDO.sh < entrada.txt"
    exit 1
fi
...
for i in {1..3}; do
    likwid-perfctr -C 0 -g FLOPS_DP -f -O "$EXEC" < /dev/stdin 2>&1 \
    | grep FP_ARITH_INST_RETIRED_SCALAR_DOUBLE | awk -F',' '{gsub(/ /, "", $2); print $1 "," $2}'
done
